                    ...
                    @foreach($account->phoneNumbers as $phoneNumber)
                        <div class="card-body box-profile">
                            <div class="user-block">
                                <img class="img-circle img-bordered-sm" src="{{ $phoneNumber->businessProfile->profile_picture_url }}" alt="user image">
                                <span class="username">
                                    <a href="#" wire:click.prevent="selectProfile({{ json_encode($phoneNumber->businessProfile) }})">{{ $phoneNumber->verified_name }}</a>
                                </span>
                                <span class="description">Phone number: +{{ $phoneNumber->display_phone_number }}</span>
                                <span class="description">Phone number ID: {{ $phoneNumber->whatsapp_business_accounts_id }}</span>
                            </div>
                        </div>
                    @endforeach
                @else
